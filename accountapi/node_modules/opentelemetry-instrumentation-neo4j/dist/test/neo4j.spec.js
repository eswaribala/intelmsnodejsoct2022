...
const instrumentation = contrib_test_utils_1.registerInstrumentationTesting(new src_1.Neo4jInstrumentation());
...
describe('neo4j instrumentation', function () {
    ...
    before(async () => {
        driver = neo4j_driver_1.default.driver('bolt://localhost:11011', neo4j_driver_1.default.auth.basic('neo4j', 'test'), {
    ...
    describe('session', () => {
        it('instruments "run" with promise', async () => {
            ...
            expect_1.default(res.records.length).toBe(1);
            expect_1.default(res.records[0].toObject().n.labels).toEqual(['MyLabel']);
            const span = getSingleSpan();
            assert_1.assertSpan(span);
            expect_1.default(span.name).toBe('CREATE neo4j');
            expect_1.default(span.attributes[semantic_conventions_1.SemanticAttributes.DB_OPERATION]).toBe('CREATE');
            expect_1.default(span.attributes[semantic_conventions_1.SemanticAttributes.DB_STATEMENT]).toBe('CREATE (n:MyLabel) RETURN n');
        });
        ...
        it('handles "run" exceptions with promise', async () => {
            try {
                ...
                const span = getSingleSpan();
                expect_1.default(span.status.code).toBe(api_1.SpanStatusCode.ERROR);
                expect_1.default(span.status.message).toBe(err.message);
                return;
            }
        ...
        it('closes span when on "onKeys" event', (done) => {
            driver
                ...
                onKeys: (keys) => {
                    const span = getSingleSpan();
                    assert_1.assertSpan(span);
                    expect_1.default(keys).toEqual(['n']);
                    done();
                },
            });
        });
        it('when passing "onKeys" and onCompleted, span is closed in onCompleted, and response hook is called', (done) => {
            ...
            driver
                ...
                onCompleted: () => {
                    const span = getSingleSpan();
                    assert_1.assertSpan(span);
                    expect_1.default(span.attributes['test']).toBe('cool');
                    done();
                },
            });
        });
        it('handles multiple promises', async () => {
            ...
            const spans = contrib_test_utils_1.getTestSpans();
            expect_1.default(spans.length).toBe(3);
            for (let span of spans) {
                assert_1.assertSpan(span);
                expect_1.default(span.attributes[semantic_conventions_1.SemanticAttributes.DB_OPERATION]).toBe('MATCH');
            }
        });
        ...
        it('set module versions when config is set', async () => {
            ...
            const span = getSingleSpan();
            expect_1.default(span.attributes['module.version']).toMatch(/\d{1,4}\.\d{1,4}\.\d{1,5}.*/);
        });
        ...
        it('does capture span when ignoreOrphanedSpans is set to true and has parent span', async () => {
            ...
            instrumentation.setConfig({ ignoreOrphanedSpans: true });
            instrumentation.enable();
            const parent = api_1.trace.getTracerProvider().getTracer('test-tracer').startSpan('main');
            await api_1.context.with(api_1.trace.setSpan(api_1.context.active(), parent), () => driver.session().run('CREATE (n:MyLabel) RETURN n'));
            const spans = contrib_test_utils_1.getTestSpans();
            expect_1.default(spans.length).toBe(1);
        });
        it('responseHook works with promise', async () => {
            ...
            instrumentation.setConfig({
                responseHook: (span, response) => {
                    span.setAttribute('db.response', test_utils_1.normalizeResponse(response));
                },
            });
            instrumentation.enable();
            ...
            const span = getSingleSpan();
            assert_1.assertSpan(span);
            expect_1.default(JSON.parse(span.attributes['db.response'])).toEqual([
                {
    ...
    describe('transaction', async () => {
        it('instruments session readTransaction', async () => {
            ...
            assert_1.assertSpan(span);
            expect_1.default(span.attributes[semantic_conventions_1.SemanticAttributes.DB_OPERATION]).toBe('MATCH');
            expect_1.default(span.attributes[semantic_conventions_1.SemanticAttributes.DB_STATEMENT]).toBe('MATCH (person:Person) RETURN person.name AS name');
        });
    ...
    describe('rxSession', () => {
        ...
        it('works when piping response', (done) => {
            ...
            rxSession
                ...
                .records()
                .pipe(operators_1.map((record) => record.get('name')))
                .subscribe({
                next: () => { },
                complete: () => {
                    const span = getSingleSpan();
                    assert_1.assertSpan(span);
                    expect_1.default(span.attributes[semantic_conventions_1.SemanticAttributes.DB_STATEMENT]).toBe('MERGE (james:Person {name: $nameParam}) RETURN james.name AS name');
                    done();
                },
                error: () => { },
            });
        ...
        it('works with response hook', (done) => {
            ...
            driver
                ...
                complete: () => {
                    const span = getSingleSpan();
                    assert_1.assertSpan(span);
                    expect_1.default(span.attributes['db.response']).toBe(`[{"n":{"labels":["MyLabel"],"properties":{}}}]`);
                    done();
                },
            });
        });
    ...
    describe('reactive transaction', () => {
        ...
        it('instruments rx explicit transactions', (done) => {
            driver
                .rxSession()
                .beginTransaction()
                .pipe(operators_1.mergeMap((txc) => rxjs_1.concat(txc
                .run('MERGE (bob:Person {name: $nameParam}) RETURN bob.name AS name', {
                nameParam: 'Bob',
            })
                .records()
                .pipe(operators_1.map((r) => r.get('name'))), txc
                .run('MERGE (adam:Person {name: $nameParam}) RETURN adam.name AS name', {
                nameParam: 'Adam',
            })
                .records()
                .pipe(operators_1.map((r) => r.get('name'))), txc.commit())))
                .subscribe({
                next: () => { },
                complete: () => {
                    const spans = contrib_test_utils_1.getTestSpans();
    ...
    describe('routing mode', () => {
        ...
        before(() => {
            if (shouldCheck) {
                routingDriver = neo4j_driver_1.default.driver('neo4j://localhost:11011', neo4j_driver_1.default.auth.basic('neo4j', 'test'));
            }
        });
        after(async () => {
...
//# sourceMappingURL=neo4j.spec.js.map